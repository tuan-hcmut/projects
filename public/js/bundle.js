...
exports.logout = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3() {
  var res;
...
},{"axios":"../../node_modules/axios/index.js"}],"updateSetting.js":[function(require,module,exports) {
"use strict";
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://127.0.0.1:3000/users/updateMyPassword' : 'http://127.0.0.1:3000/users/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
...
},{"axios":"../../node_modules/axios/index.js"}],"sudoku.js":[function(require,module,exports) {
"use strict";
...
var markRow = new Array(9);
var markCol = new Array(9);
var boardChild = new Array(3);
...
for (var _i = 0; _i < 3; _i++) {
  boardChild[_i] = new Array(3);
}

for (var _i2 = 0; _i2 < 3; _i2++) {
  for (var j = 0; j < 3; j++) {
    boardChild[_i2][j] = new Array(9);
  }
}

for (var _i3 = 0; _i3 < 9; _i3++) {
  for (var _j = 0; _j < 9; _j++) {
    markRow[_i3][_j] = false;
    markCol[_i3][_j] = false;
  }
}
...
var numInContainers = function numInContainers(val, j) {
  var _loop = function _loop(_i5) {
    document.getElementById('board').children[_i5].addEventListener('click', function (e) {
      e.preventDefault();

      if (_i5 < 9) {
        board[0][_i5] = Number(val);
      } else {
        board[Math.floor(_i5 / 9)][_i5 - Math.floor(_i5 / 9) * 9] = Number(val);
      }

      document.getElementById('board').children[_i5].textContent = String(val);
      document.getElementById('number-container').children[j].classList.remove('selected');
    });
  };

  for (var _i5 = 0; _i5 < document.getElementById('board').children.length; _i5++) {
    _loop(_i5);
  }
};
...
var check = function check() {
  var _loop2 = function _loop2(_i6) {
    document.getElementById('number-container').children[_i6].addEventListener('click', function (e) {
      e.preventDefault();

      document.getElementById('number-container').children[_i6].classList.add('selected');

      window.setTimeout(function () {}, 1000);
      numInContainers(document.getElementById('number-container').children[_i6].textContent, _i6);
    });
  };

  for (var _i6 = 0; _i6 < document.getElementById('number-container').children.length; _i6++) {
    _loop2(_i6);
  }
}; // board = [
//   [2, 0, 0, 0, 4, 9, 0, 8, 3],
...
for (var _i7 = 0; _i7 < 9; _i7++) {
  for (var _j3 = 0; _j3 < 9; _j3++) {
    if (board[_i7][_j3] !== 0) {
      var num = board[_i7][_j3];
      markRow[_i7][num - 1] = true;
      markCol[_j3][num - 1] = true;
      boardChild[Math.floor(_i7 / 3)][Math.floor(_j3 / 3)][num - 1] = true;
    }
  }
}

var solve = function solve(i, j, board) {
  if (i < 9 && j < 9) {
    if (board[i][j] === 0) {
      for (var _z = 1; _z <= 9; _z++) {
        if (!markCol[j][_z - 1] && !markRow[i][_z - 1] && !boardChild[Math.floor(i / 3)][Math.floor(j / 3)][_z - 1]) {
          markRow[i][_z - 1] = true;
          markCol[j][_z - 1] = true;
          boardChild[Math.floor(i / 3)][Math.floor(j / 3)][_z - 1] = true;
          board[i][j] = _z; //   console.log(board[i][j]);

          solve(i, j + 1, board);
          markRow[i][_z - 1] = false;
          markCol[j][_z - 1] = false;
          boardChild[Math.floor(i / 3)][Math.floor(j / 3)][_z - 1] = false;
          board[i][j] = 0;
        }
      }
    } else {
    ...
    for (var _i8 = 0; _i8 < document.getElementById('board').children.length; _i8++) {
      if (_i8 < 9) {
        document.getElementById('board').children[_i8].textContent = board[0][_i8];
      } else {
        document.getElementById('board').children[_i8].textContent = board[Math.floor(_i8 / 9)][_i8 - Math.floor(_i8 / 9) * 9];
      }
    }
  }
};
  ...
  solveBtn.addEventListener('click', function (e) {
    ...
    for (var _i9 = 0; _i9 < 9; _i9++) {
      for (var _j4 = 0; _j4 < 9; _j4++) {
        if (board[_i9][_j4] !== 0) {
          var _num = board[_i9][_j4];
          markRow[_i9][_num - 1] = true;
          markCol[_j4][_num - 1] = true;
          boardChild[Math.floor(_i9 / 3)][Math.floor(_j4 / 3)][_num - 1] = true;
        }
      }
    }
...
var sudokuGame = document.querySelector('.newgame-btn');
      ...
      return regeneratorRuntime.wrap(function _callee4$(_context4) {
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              e.preventDefault();
              document.querySelector('.btn--save-password').textContent = 'Updating...'; /// passwordCurrent .... will be get from html a then these date will be send to the link url in axios along with req.body
